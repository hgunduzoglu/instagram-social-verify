 ...
 */ export const deployRollup = async (extendedClient, deployer, args, addresses, logger)=>{
    ...
    if (args.initialValidators) {
        await cheat_initializeValidatorSet(extendedClient, deployer, rollupAddress.toString(), addresses.stakingAssetAddress.toString(), args.initialValidators.map((v)=>v.toString()), args.acceleratedTestDeployments, logger);
    }
...
export const handoverToGovernance = async (extendedClient, deployer, registryAddress, governanceAddress, logger, acceleratedTestDeployments)=>{
    // We need to call a function on the registry to set the various contract addresses.
    const registryContract = getContract({
...
export const cheat_initializeValidatorSet = async (extendedClient, deployer, rollupAddress, stakingAssetAddress, validators, acceleratedTestDeployments, logger)=>{
    ...
    if (validators && validators.length > 0) {
        ...
        if (!acceleratedTestDeployments) {
            const validatorsInfo = await Promise.all(validators.map(async (address)=>({
                    address,
        ...
        if (validators.length > 0) {
            ...
            ].map((tx)=>extendedClient.waitForTransactionReceipt({
                    hash: tx.txHash
                })));
            ...
            await extendedClient.waitForTransactionReceipt({
                hash: initiateValidatorSetTxHash
            });
 ...
 */ export const deployL1Contracts = async (rpcUrls, account, chain, logger, args, txUtilsConfig = getL1TxUtilsConfigEnvVars())=>{
    ...
    const { feeAssetAddress, feeAssetHandlerAddress, stakingAssetAddress, stakingAssetHandlerAddress, registryAddress, rewardDistributorAddress } = await deploySharedContracts(l1Client, deployer, args, logger);
...
export class L1Deployer {
    ...
    async waitForDeployments() {
        ...
        await Promise.all(this.txHashes.map((txHash)=>this.client.waitForTransactionReceipt({
                hash: txHash
            })));
 ...
 */ export async function deployL1Contract(extendedClient, abi, bytecode, args = [], maybeSalt, libraries, logger, l1TxUtils, acceleratedTestDeployments = false) {
    ...
    if (libraries) {
        ...
        for(const libraryName in libraries?.libraryCode){
            ...
            const { address, txHash } = await deployL1Contract(extendedClient, lib.contractAbi, lib.contractBytecode, [], maybeSalt, undefined, logger, l1TxUtils, acceleratedTestDeployments);
            if (txHash) {
    ...
    if (maybeSalt) {
        ...
        const existing = await extendedClient.getBytecode({
