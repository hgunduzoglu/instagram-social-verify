 ...
 */
function findSelectorInCleanContent(cleanContent, selector, positionMap) {
 ...
 * returns: "Hel=\r\nlo"
 */
function getAdjustedSelector(originalBody, selector, cleanContent, positionMap) {
...
function removeSoftLineBreaks(body) {
    const result = [];
    const positionMap = new Map(); // clean -> original
    let i = 0;
    let cleanPos = 0;
    while (i < body.length) {
        if (i + 2 < body.length &&
...
function generateEmailVerifierInputsFromDKIMResult(dkimResult, params = {}) {
    ...
    if (!params.ignoreBodyHashCheck) {
        ...
        if (params.removeSoftLineBreaks) {
            ...
            circuitInputs.decodedEmailBodyIn = (0, binary_format_1.Uint8ArrayToCharArray)(cleanContent);
        }
