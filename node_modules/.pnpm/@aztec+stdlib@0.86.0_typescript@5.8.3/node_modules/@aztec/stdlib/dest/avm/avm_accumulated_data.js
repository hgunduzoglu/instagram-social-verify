...
export class AvmAccumulatedData {
    ...
    static fromBuffer(buffer) {
        ...
        return new this(reader.readArray(MAX_NOTE_HASHES_PER_TX, Fr), reader.readArray(MAX_NULLIFIERS_PER_TX, Fr), reader.readArray(MAX_L2_TO_L1_MSGS_PER_TX, ScopedL2ToL1Message), reader.readArray(MAX_PUBLIC_LOGS_PER_TX, PublicLog), reader.readArray(MAX_TOTAL_PUBLIC_DATA_UPDATE_REQUESTS_PER_TX, PublicDataWrite));
    }
    toBuffer() {
    ...
    static fromFields(fields) {
        ...
        return new this(reader.readFieldArray(MAX_NOTE_HASHES_PER_TX), reader.readFieldArray(MAX_NULLIFIERS_PER_TX), reader.readArray(MAX_L2_TO_L1_MSGS_PER_TX, ScopedL2ToL1Message), reader.readArray(MAX_PUBLIC_LOGS_PER_TX, PublicLog), reader.readArray(MAX_TOTAL_PUBLIC_DATA_UPDATE_REQUESTS_PER_TX, PublicDataWrite));
    }
    ...
    static empty() {
        return new this(makeTuple(MAX_NOTE_HASHES_PER_TX, Fr.zero), makeTuple(MAX_NULLIFIERS_PER_TX, Fr.zero), makeTuple(MAX_L2_TO_L1_MSGS_PER_TX, ScopedL2ToL1Message.empty), makeTuple(MAX_PUBLIC_LOGS_PER_TX, PublicLog.empty), makeTuple(MAX_TOTAL_PUBLIC_DATA_UPDATE_REQUESTS_PER_TX, PublicDataWrite.empty));
    }
    isEmpty() {
        return this.noteHashes.every((x)=>x.isZero()) && this.nullifiers.every((x)=>x.isZero()) && this.l2ToL1Msgs.every((x)=>x.isEmpty()) && this.publicLogs.every((x)=>x.isEmpty()) && this.publicDataWrites.every((x)=>x.isEmpty());
    }
    [inspect.custom]() {
  ...
  publicLogs: [${this.publicLogs.filter((x)=>!x.isEmpty()).map((h)=>inspect(h)).join(', ')}],
