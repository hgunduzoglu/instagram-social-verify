...
function pickSmallestRemainder(options) {
    const optionsWithSize = options.filter((o)=>o.remainder).map((option)=>({
            option,
            size: getSize(option.remainder)
        }));
...
function findStandalone(requestedDimensions, config, isQualified) {
    ...
    const dimensions = PrivateKernelResetDimensions.from({
        [name]: value
    });
...
export function findPrivateKernelResetDimensions(requestedDimensions, config, isInner = false, allowRemainder = false) {
    ...
    const isEnough = allowRemainder ? ()=>true : (dimensions)=>dimensions.toValues().every((v, i)=>v >= requestedValues[i]);
    const isQualified = !isInner ? isEnough : (dimensions)=>dimensions.NOTE_HASH_SILOING_AMOUNT === 0 && dimensions.NULLIFIER_SILOING_AMOUNT === 0 && dimensions.PRIVATE_LOG_SILOING_AMOUNT === 0 && isEnough(dimensions);
    const options = [
