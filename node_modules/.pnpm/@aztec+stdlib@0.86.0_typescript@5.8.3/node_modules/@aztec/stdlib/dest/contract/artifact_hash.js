 ...
 */ export async function computeArtifactHash(artifact) {
    ...
    getLogger().trace('Computed artifact hash', {
        artifactHash,
        ...preimage
    });
    return artifactHash;
...
export function computeArtifactMetadataHash(artifact) {
    return sha256Fr(Buffer.from(JSON.stringify({
        name: artifact.name,
...
async function computeFunctionLeaves(artifact, fnType) {
    const selectors = await Promise.all(artifact.functions.filter((f)=>f.functionType === fnType).map(async (f)=>({
            ...f,
...
export async function computeFunctionArtifactHash(fn) {
...
export function getArtifactMerkleTreeHasher() {
    return (l, r)=>Promise.resolve(sha256Fr(Buffer.concat([
            l,
            r
        ])).toBuffer());
}
