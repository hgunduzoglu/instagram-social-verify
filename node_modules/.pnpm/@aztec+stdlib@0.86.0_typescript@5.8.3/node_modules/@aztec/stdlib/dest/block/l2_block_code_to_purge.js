...
/**
 * Makes header.
 */ export function makeHeader(seed = 0, numTxs = undefined, blockNumber = undefined, slotNumber = undefined, inHash = undefined) {
    return new BlockHeader(makeAppendOnlyTreeSnapshot(seed + 0x100), makeContentCommitment(seed + 0x200, numTxs, inHash), makeStateReference(seed + 0x600), makeGlobalVariables(seed += 0x700, blockNumber, slotNumber ?? blockNumber), new Fr(seed + 0x800), new Fr(seed + 0x900));
}
/**
 ...
 */ function makeContentCommitment(seed = 0, numTxs = undefined, inHash = undefined) {
    return new ContentCommitment(numTxs ? new Fr(numTxs) : new Fr(seed), toBufferBE(BigInt(seed + 0x100), NUM_BYTES_PER_SHA256), inHash ?? toBufferBE(BigInt(seed + 0x200), NUM_BYTES_PER_SHA256), toBufferBE(BigInt(seed + 0x300), NUM_BYTES_PER_SHA256));
}
/**
