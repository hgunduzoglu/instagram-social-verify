import { CONTRACT_CLASS_LOGS_PREFIX, CONTRACT_CLASS_LOG_SIZE_IN_FIELDS, L2_L1_MSGS_PREFIX, MAX_CONTRACT_CLASS_LOGS_PER_TX, MAX_L2_TO_L1_MSGS_PER_TX, MAX_NOTE_HASHES_PER_TX, MAX_NULLIFIERS_PER_TX, MAX_PRIVATE_LOGS_PER_TX, MAX_TOTAL_PUBLIC_DATA_UPDATE_REQUESTS_PER_TX, NOTES_PREFIX, NULLIFIERS_PREFIX, PRIVATE_LOGS_PREFIX, PRIVATE_LOG_SIZE_IN_FIELDS, PUBLIC_DATA_UPDATE_REQUESTS_PREFIX, PUBLIC_LOGS_PREFIX, PUBLIC_LOG_SIZE_IN_FIELDS, REVERT_CODE_PREFIX, TX_FEE_PREFIX } from '@aztec/constants';
...
export class TxEffect {
   ...
   */ static fromBuffer(buffer) {
        ...
        return new TxEffect(RevertCode.fromBuffer(reader), TxHash.fromBuffer(reader), Fr.fromBuffer(reader), reader.readVectorUint8Prefix(Fr), reader.readVectorUint8Prefix(Fr), reader.readVectorUint8Prefix(Fr), reader.readVectorUint8Prefix(PublicDataWrite), reader.readVectorUint8Prefix(PrivateLog), reader.readVectorUint8Prefix(PublicLog), reader.readVectorUint8Prefix(ContractClassLog));
    }
    /**
   ...
   */ toBlobFields() {
        ...
        flattened.push(new Fr(Buffer.concat([
            Buffer.from([
                TX_FEE_PREFIX
            ]),
            Buffer.alloc(1),
            this.transactionFee.toBuffer().subarray(3)
        ])));
        ...
        if (this.publicDataWrites.length) {
            ...
            flattened.push(...this.publicDataWrites.map((w)=>[
                    w.leafSlot,
                    w.value
                ]).flat());
        }
   ...
   */ static fromBlobFields(fields) {
        ...
        effect.transactionFee = Fr.fromBuffer(Buffer.concat([
            Buffer.alloc(3),
            prefixedFee.toBuffer().subarray(3)
        ]));
                ...
                case CONTRACT_CLASS_LOGS_PREFIX:
                    {
                        ...
                        while(i < length){
                            ...
                            effect.contractClassLogs.push(ContractClassLog.fromFields(logFields.concat(new Array(CONTRACT_CLASS_LOG_SIZE_IN_FIELDS - logLen).fill(Fr.ZERO))));
                        }
                        break;
                    }
