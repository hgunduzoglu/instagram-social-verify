 ...
 * @returns
 */
async function verifyDKIMSignature(email, domain = '', enableSanitization = true, fallbackToZKEmailDNSArchive = false, skipBodyHash = false) {
    ...
    if (dkimResult.status.comment === 'bad signature' && enableSanitization) {
        const results = await Promise.all(sanitizers_1.default.map((sanitize) => tryVerifyDKIM(sanitize(emailStr), domain, fallbackToZKEmailDNSArchive, skipBodyHash).then((result) => ({
            result,
...
async function tryVerifyDKIM(email, domain = '', fallbackToZKEmailDNSArchive = false, skipBodyHash = false) {
